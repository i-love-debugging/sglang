...
class AdaptiveAllReduceManager:
    ...
    def initialize_communicators(self):
        ...
        if world_size <= 1:
            logger.info("Single GPU, skipping adaptive allreduce communicators initialization")
            return
        ...
        try:
            ...
            if not self.torch_symm_mem_communicator.disabled:
                logger.info("Torch symmetric memory communicator initialized for adaptive allreduce")
            else:
                logger.info("Torch symmetric memory communicator is disabled")
                self.torch_symm_mem_communicator = None
        except Exception as e:
            logger.warning(f"Failed to initialize torch symmetric memory communicator: {e}")
            self.torch_symm_mem_communicator = None
